                    ...
                    <?php// include 'old_navbar.inc.php'; ?>
                     ...
                     <div class="collapse navbar-collapse" id="myNavbar">
                         ...
                         /*
                            ...
                            $s = $db->prepare($sql);
                            $s->execute();
                            $main_menu = $s->fetchAll(PDO::FETCH_ASSOC);
                            
                            $sql = "SELECT * FROM sub_menu";
                            $s = $db->prepare($sql);
                            $s->execute();
                            $sub_menu = $s->fetchAll(PDO::FETCH_ASSOC);                       
                         ?>

                        <ul class="nav navbar-nav">
                            <?php foreach($main_menu as $item): ?>
                            <li>
                                <a href="?goto=<?php echo htmlspecialchars($item['main_menu_name']); ?>"><?php echo htmlspecialchars($item['main_menu_name']); ?></a>
                                <?php endforeach; ?>
                                
                                <ul>
                                    <?php foreach($sub_menu as $item): ?>
                                    <li>
                                        <a href="?goto=<?php echo htmlspecialchars($item['sub_menu_name']); ?>"><?php echo htmlspecialchars($item['sub_menu_name']); ?></a>
                                    </li>
                                    <?php endforeach; ?>
                                </ul>
                            </li>
                         ...
                         <ul class="nav navbar-nav">
                            ...
                            $res = $db->query("SELECT * FROM main_menu");
                                while($row=$res->fetch(PDO::FETCH_ASSOC)){?>
                                    ...
                                    <?php $res_pro = $db->query("SELECT * FROM sub_menu WHERE main_menu_id=".$row['main_menu_id']);?>
                                    
                                        <ul class="dropdown-menu text-capitalize">    
                                            <?php  
                                                while($pro_row=$res_pro->fetch(PDO::FETCH_ASSOC)){?>

                                            <li><a href="index.php?get_page=<?php echo $pro_row['sub_menu_name']; ?>&amp;id=<?php echo $pro_row['page_id']; ?>"><?php echo $pro_row['sub_menu_name']; ?></a></li>
                                            <?php }?>
                                        </ul>
                                    </li> <?php
                                }
